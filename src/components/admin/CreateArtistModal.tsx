...
export default function CreateArtistModal() {
  ...
  const [createState, setCreateState] = useState<{ error?: string; success?: boolean; message?: string } | null>(null);
  ...
  async function handleCreateArtist(formData: FormData) {
    ...
    try {
        const result = await createArtist(null, formData);
        if (result.success) {
            ...
            <div className="bg-[#1a1a1a] border border-white/10 rounded-xl p-6 w-full max-w-md relative">
                ...
                <form action={handleCreateArtist} className="space-y-4">
                    <div>
                        ...
                        <input name="name" type="text" required className="w-full bg-black/20 border border-white/10 rounded-lg px-3 py-2" placeholder="Stage Name" />
                    </div>
                    <div>
                        <label className="text-sm font-medium block mb-1">Email</label>
                        <input name="email" type="email" required className="w-full bg-black/20 border border-white/10 rounded-lg px-3 py-2" placeholder="artist@example.com" />
                    </div>
                    <div>
                        <label className="text-sm font-medium block mb-1">Password</label>
                        <input name="password" type="password" required className="w-full bg-black/20 border border-white/10 rounded-lg px-3 py-2" placeholder="Temporary Password" />
                    </div>
                    <button 
                        type="submit" 
                        disabled={loading}
