...
export default function TrackPage({
  ...
  const breadcrumbItems =
    role === 'admin'
      ? [
          { label: 'Artists', href: '/admin' },
          {
            label: artistName || 'Unknown Artist',
            href: track.artistId ? `/admin/artist/${track.artistId}` : '#',
          },
          { label: track.title },
        ]
      : [{ label: 'Tracks', href: '/dashboard' }, { label: track.title }]
        ...
        <div className="flex items-center gap-4">
          <div className="relative">
            ...
            >
              ...
              <ChevronDown
                size={16}
                className={`transition-transform ${isVersionsOpen ? 'rotate-180' : ''}`}
              />
            </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-6">
            ...
            <p className="text-muted-foreground text-sm leading-relaxed">
              {activeVersion
                ? `Uploaded on ${format(new Date(activeVersion.createdAt), 'dd MMMM yyyy')}`
        ...
        <div className="bg-white/5 border border-white/10 rounded-xl p-6 h-fit">
          <div className="flex items-center justify-between mb-6">
            ...
            <button
              onClick={openMainCommentModal}
              className="p-2 hover:bg-white/10 rounded-full text-primary transition-colors"
            >
              <Plus size={20} />
            </button>
          </div>
