    ...
    >
      <div className="flex gap-4">
        ...
        <div className="flex-1">
          <h2 className="font-title text-lg font-semibold tracking-tight">{artist.name}</h2>
...
export default function Home() {
  ...
  const keyValues = tHome.raw('keyValues') as Array<{ title: string; description: string }>

  const [bioImage, setBioImage] = useState('/images/shinji_home_studio.jpg')
  ...
  useEffect(() => {
    fetch('/api/content?page=bio', { cache: 'no-store' })
      ...
      .then(data => { if (data?.content?.image) setBioImage(data.content.image) })
      .catch(console.error)

    fetch('/api/content?page=work', { cache: 'no-store' })
      .then(res => res.ok ? res.json() : null)
      .then(data => { if (data?.content?.artists) setArtists(data.content.artists) })
      .catch(console.error)
      .finally(() => setLoadingArtists(false))
  }, [])

  const isExternalImage = bioImage.startsWith('http')
    ...
    <div className="font-sans">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          ...
          >
            {isExternalImage ? (
              ...
              <Image src={bioImage} alt={tBio('title')} fill className="object-cover" />
            )}
          </motion.div>
      ...
      <section id="work" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-24">
        ...
        {!loadingArtists && artists.length === 0 && (
          <p className="text-muted-foreground">{tWork('comingSoon')}</p>
        )}
      ...
      <section id="contact" className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
        ...
        >
          <div className="text-center space-y-4">
            <h2 className="font-title text-4xl font-bold tracking-tight">{tContact('heading')}</h2>
            <p className="font-caption text-muted-foreground">{tContact('subtext')}</p>
          </div>

          <form className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label htmlFor="name" className="font-caption text-sm font-medium">
                  {tContact('nameLabel')}
                </label>
                <input
                  id="name"
              ...
              <div className="space-y-2">
                <label htmlFor="email" className="font-caption text-sm font-medium">
                  {tContact('emailLabel')}
                </label>
                <input
                  id="email"
            ...
            <div className="space-y-2">
              <label htmlFor="message" className="font-caption text-sm font-medium">
            ...
            >
              {tContact('sendButton')} <Send size={18} />
            </button>
          </form>
        </motion.div>
      </section>
