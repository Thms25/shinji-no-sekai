// forked from design-sync lib/source-kit.mjs — Next.js app-router paths are routing
// structure, not design taxonomy: src/app/[locale]/... derived groups like "locale"
// and "site-content". Under app/, yield 'general' so the doc frontmatter category wins.
...
export async function resolvePackage(ctx) {
  ...
  if (srcRoot) {
    for (const c of components) {
      ...
      const relDir = slash(relative(srcRoot, dirname(hit)));
      // FORK: anything under the Next app/ tree is a route path (app/[locale]/admin/
      // site-content/...), which says nothing about design grouping. Emit 'general' so
      // package-build's doc-category pass (which only overrides general/misc) applies.
      c.group = /^app(\/|$)/.test(relDir)
        ? 'general'
        : slug(
            relDir
              .split('/')
